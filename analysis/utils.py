...
def plot_stock_data(df):
    ...
    fig.add_trace(go.Scatter( # Upper Bound
        name='High',
    ...
    fig.add_trace(go.Scatter( # Lower Bound
        name='Low',
...
def plot_forecasting(original: pd.Series, fitted_data: pd.Series, predictions: pd.Series, title, zoom: bool=False,save: bool=False, n_days:int=90):
    fig = go.Figure()
    ...
    fig.update_xaxes(title='days',rangeslider_visible=True,
        rangeselector=dict(
        buttons=list([
    ...
    if zoom:
        ...
        xaxis_dict=dict(range=[datetime.datetime(2018, 1, 1),
            datetime.datetime(new_ts.year, new_ts.month, new_ts.day)])
